   ...
   <div class="app-container">
      <el-form :model="formInline" :inline="true" style="margin-left: 0.2%;">
         <el-form-item label="">
               ...
               <el-select v-model="queryParams.payment" placeholder="选择状态" style="width: 180px;" clearable >
                  <el-option label="款项未收齐" value="1" />
                  <el-option label="款项已结清" value="2" />
                  <el-option label="发票未开完" value="3" />
                  <el-option label="发票已开完" value="4" />
               </el-select>
         </el-form-item>
         <el-form-item>
               ...
               <el-button @click="paymentFun">款项发票已结清</el-button>
      ...
      <el-table :data="tableList" border style="width: 100%" @selection-change="handleSelectionChange">
         ...
         <el-table-column label="未付款(元)" align="center" prop="casePaidsal" />
         <el-table-column label="已开票(元)" align="center" prop="caseInvoiced"/>
         <el-table-column label="未开票(元)" align="center">
...
import { myStayProceedsCase, editCaseLaw, myStayYear, setPayment } from "@/api/cases/CaseLaw.js";
...
//   console.log(multipleSelection.value[val.length-1].id);
//   const mult = reactive({
//       id: '',
//       casePaidsal: '',
//       caseAgencyfee: '',
...
//       caseNotinvoiced: '',
      // id: val.id,
      // casePaidsal: val.casePaidsal,
      // caseAgencyfee: val.caseAgencyfee,
      // caseProxysal: val.caseProxysal,
      // caseFxmoney: val.caseFxmoney,
      // caseUnpaidsal: val.caseUnpaidsal,
      // caseInvoiced: val.caseInvoiced,
      // caseNotinvoiced: val.caseNotinvoiced,
//   });
//   arrs.push(mult);
}
...
function paymentFun(){
   for(var i = 0; i < multipleSelection.value.length; i++){
      const mult = reactive({
         ...
         casePaidsal: multipleSelection.value[i].casePaidsal == null ? 0 : multipleSelection.value[i].casePaidsal,
         caseAgencyfee: multipleSelection.value[i].caseAgencyfee == null ? 0 : multipleSelection.value[i].caseAgencyfee,
         caseProxysal: multipleSelection.value[i].caseProxysal == null ? 0 : multipleSelection.value[i].caseProxysal,
         caseFxmoney: multipleSelection.value[i].caseFxmoney == null ? 0 : multipleSelection.value[i].caseFxmoney,
         caseUnpaidsal: multipleSelection.value[i].caseUnpaidsal == null ? 0 : multipleSelection.value[i].caseUnpaidsal,
         caseInvoiced: multipleSelection.value[i].caseInvoiced == null ? 0 : multipleSelection.value[i].caseInvoiced,
         caseNotinvoiced: multipleSelection.value[i].caseNotinvoiced == null ? 0 : multipleSelection.value[i].caseNotinvoiced,
      });
      str = str +  JSON.stringify(mult);
   }
   setPayment(str).then(response => {
...
function queryParamsSubmit(){
   getQueryParams();
   myStayProceedsCase(proxy.addDateRange(queryParams)).then(response => {
