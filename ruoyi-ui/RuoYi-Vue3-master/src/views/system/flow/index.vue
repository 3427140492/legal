<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="${comment}" prop="caseTypeId">
        <el-input
          v-model="queryParams.caseTypeId"
          placeholder="请输入${comment}"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="${comment}" prop="actDeploymentId">
        <el-input
          v-model="queryParams.actDeploymentId"
          placeholder="请输入${comment}"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="flowList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="${comment}" align="center" prop="caseTypeId" />
      <el-table-column label="${comment}" align="center" prop="actDeploymentId" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="${comment}" prop="caseTypeId">
          <el-input v-model="form.caseTypeId" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="${comment}" prop="actDeploymentId">
          <el-input v-model="form.actDeploymentId" placeholder="请输入${comment}" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除floww编号为"' + ids + '"的数据项？').then(function() {
        return delFlow(ids);
      }).then(() => {
        this.getList();
